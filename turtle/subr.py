...
def expr_regen_cond(bt, stag, tag, expr, idx):
    '''
    re-generation of 'cond' expression in strategy class
    ...
    Input 
        ...
        - idx     : Date index
    Out
        - result for expression [True / False]
    '''
    # Decompose expression (LHS, RHS, operator)
    lhs, rhs, oper = split_expression(expr)
    
    # CHECK FOR SPECIAL CASE: 전략트리 입력(ecnt / dcnt) 확인
    if re.search(r"ecnt|dcnt", lhs):
        
        # Check external tag call
        if '[' in lhs:
        
            # Decompose LHS
            lhs, stag, tag = re.match(r'(\w+)\[([\w#]+),([\w#]+)\]', lhs).groups()
        
        # Access LHS
        lhs_val = getattr(bt._st, stag).find_sub_by_tag(tag).res[lhs]
        
        # Modified expression
        res = (str(lhs_val) + oper + rhs)            
    
    
    # FOR GENERAL CASE: 전략트리 입력(ecnt / dcnt)이외의 입력 (sgnl과 동일한 형태. operator 만 다름)
    else:
        
        # Modify expression
        lhs, _ = expr_regen(bt, stag, lhs, {}, idx)
