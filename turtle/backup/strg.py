...
class strategy:
    ...
    # LIMIT BUY
    def buy_lim(self, tag, sgnl, pric, amnt, lamt=1, retr=0, **kwargs):
    ...
    # LIMIT SEL
    def sel_lim(self, tag, sgnl, pric, amnt, lamt=1, retr=0, **kwargs):
        # Prepare dynamic arguments (excluding 'self')
        args = {k: v for k, v in locals().items() if k != 'self'}  # Exclude 'self'
    ...
    # LOC BUY
    def buy_loc(self, tag, bprc, amnt, lamt=1, retr=0, **kwargs):
        '''
        bprc : LOC 주문 기준가
             : 종가 < 기준가 시 종가 매수
    ...
    # LOC SEL
    def sel_loc(self, tag, bprc, amnt, lamt=1, retr=0, **kwargs):
        '''
        sgnl : LOC 주문 기준가
             : 종가 > 기준가 시 종가 매도
    ...
    # INDIVIDUAL STRATEGY            
    def set_sig_idv(self, stag, stus, tag, sgnl, pric, otyp, amnt, lamt=1, retr=0, **kwargs):
        '''
        Set individual strategy
             ...
             pric   : price for 'sgnl' is True (ex. 'High20')
                    : pric='pric' -> sgnl=True 일 때의 가격을 의미
             ...
             retr   : Repeated trading flag (for each cycle)
                    : 0 - 싸이클당 한 번만 적용 (default)
                    : 1 - 싸이클당 (조건 충족 시) 중복 적용
                   
             **kwargs : variables used in signal
                      : format: A=a, B=b, ...
        '''
        # Init. tree class
        tree = strg_tree(tag=tag, styp='idv', stus=stus, sgnl=sgnl, pric=pric, otyp=otyp,
                         amnt=amnt, lamt=lamt * self._pf.aloc[stag]['amax'], retr=retr, **kwargs)
...
# method
#   - gen_istg_df : 개별 전략 df 생성
#   - gen_sstg_df : 셋 전략 df 생성



# =============================================================================        
# CLASS FOR STRATEGY TREE
# =============================================================================
class strg_tree:
    ...
    def __init__(self, tag=[], styp=[], **kwargs):
        ...
        # Evaluation results
        # ecnt : 1투자 싸이클당 총 실행횟수 -> retr 옵션 확인에 사용
        # dcnt : 1거래일당 ecnt변화-필터링목적 -> ceky 옵션 확인에 사용
        self.res = {
    ...
    def display(self, level=0):
        ...
        # For inside the tree
            ...
            if self.styp == 'idv':
                if self.strg == True:
                    # For True
                    print(f'{Fore.CYAN}{indent}[{self.tag}] {indent2} ecnt={self.ecnt},dcnt={self.dcnt},retr={self.arg["retr"]}{Style.RESET_ALL}')
                else:
                    # For False
                    print(f'{indent}[{self.tag}] {indent2} ecnt={self.ecnt},dcnt={self.dcnt},retr={self.arg["retr"]}')
